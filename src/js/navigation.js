(function (root, factory) {
  ...
  function Plugin(element, options) {
    ...
    nav.init = function (options) {
      ...
      nav.menuItems = nav.querySelectorAll(
        ".navigation-item, .navigation-dropdown-item"
      );
      nav.menuLinks = nav.querySelectorAll(
        ".navigation-link, .navigation-dropdown-link"
      );

      var submenus = nav.querySelectorAll(
      ...
      if (nav.options.autoSubmenuIndicator) {
        for (var i = 0; i < nav.menuItems.length; i++) {
          if (nav.menuItems[i].classList.contains("has-submenu")) {
            ...
            if (
              nav.menuItems[i].children[1].classList.contains(
    ...
    var navigationMode = function () {
      ...
      if (
        windowWidth() < nav.options.breakpoint &&
        bigScreenFlag > nav.options.breakpoint
      ) {
      ...
      if (
        windowWidth() > nav.options.breakpoint &&
        smallScreenFlag < nav.options.breakpoint
      ) {
    ...
    nav.toggleOffcanvas = function () {
      if (!nav.classList.contains(nav.options.landscapeClass)) {
        if (
          !nav.navigationBody.classList.contains("is-visible") &&
          !nav.classList.contains(nav.options.landscapeClass)
        ) {
    ...
    nav.hideSubmenus = function (el) {
      ...
      if (el === "BODY") {
        ...
        submenus = el.parentNode.querySelectorAll(
      ...
      for (var i = submenus.length - 1; i >= 0; i--) {
        ...
        if (
          submenus[i].previousElementSibling.getElementsByClassName(
            "submenu-indicator"
          ).length > 0
        ) {
          submenus[i].previousElementSibling.lastElementChild.classList.remove(
            "is-active"
          );
        }
      }
        ...
        nav.menuLinks[i].on(eventName, function (e) {
          if (e.target.parentNode.classList.contains("has-submenu")) {
            ...
            if (e.target.parentNode.classList.contains("is-active")) {
              ...
              if (
                e.target.getElementsByClassName("submenu-indicator").length > 0
              ) {
            ...
            if (
              ...
              e.target.getAttribute("target") === "blank"
            ) {
    ...
    var fixSubmenuRightPositionAll = function () {
      var submenus = nav.navigationBody.querySelectorAll(
        ".navigation-item > .navigation-submenu"
      );
      var navWidth = nav.navigationBody.offsetWidth;
      for (var i = 0; i < submenus.length; i++) {
        if (
          submenus[i].previousElementSibling.offsetLeft +
            submenus[i].offsetWidth >
          navWidth
        ) {
      ...
      var smoothScroll = function (destination, duration, easing, callback) {
        ...
        var destinationOffset =
          typeof destination === "number"
            ? destination
            : destination.offsetTop + nav.options.scrollSpyOffset;
